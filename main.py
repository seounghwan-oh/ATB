...
async def startup():
    log_message(f"ATB Start! - Version:{VERSION}")


@app.on_event("shutdown")
async def shutdown():
    log_message(f"ATB Stop! - Version:{VERSION}")
    db.close()

# Link : https://www.tradingview.com/support/solutions/43000529348-about-webhooks/
whitelist = [
    "52.89.214.238",    # TradingView IP 1
    "34.212.75.30",     # TradingView IP 2
    "54.218.53.128",    # TradingView IP 3
    "52.32.178.7",      # TradingView IP 4
    "127.0.0.1",
]
whitelist = whitelist + (settings.WHITELIST or [])
